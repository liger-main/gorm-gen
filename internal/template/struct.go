...
const (
	...
	func ({{.S}} {{.QueryStructName}}) AllColumnsForInsert() gen.Columns { return {{.S}}.{{.QueryStructName}}Do.AllColumnsForInsert({{.S}}.FieldMap) }
...
type I{{.ModelStructName}}Do interface {
	...
	OnUniqueConflictDoNothing(model interface{}) gen.Dao
